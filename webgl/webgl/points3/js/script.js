...
onload = function(){
	...
	var torusData = createDNA(8);// createPoint(50000);	// torus(64, 64, 1.5, 3.0);
	var position = torusData.p;
	var LL = position.length/3;
	...
	(function(){
		...
		m.identity(mMatrix);
		// m.rotate(mMatrix, count, [1,1,0], mMatrix);
		m.rotate(mMatrix, count, [1,0,0], mMatrix);
	...
	function createPoint(num){
		var pos = new Array(),size = new Array(),radius = 6.0,toRAD = Math.PI/180.0;
	...
	function createDNA(zIndex){
		var pos = new Array(),size = new Array(),radius = 6.0,toRAD = Math.PI/180.0,
			degX = toRAD*randomRange(0,360),
			degY = toRAD*randomRange(0,360),
			degZ = toRAD*randomRange(0,360);

		for(var i=-zIndex;i<zIndex;i+=0.01){
			var rad = i*80*toRAD;
			for(var ii=-20;ii<20;ii+=0.6){
				var r = ii*radius/40,
					y = Math.cos(rad)*r,
					z = -Math.sin(rad)*r,
					x = i*5,
