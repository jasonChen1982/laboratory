...
function ParticleCricle(){
  ...
  this.speed = JC.random(1,2);
  this.alphaEnd = JC.random(0.5,1);

  var degree = JC.random(0,360);
  this.directionX = Math.sin(degree * JC.DTR);
  this.directionY = Math.cos(degree * JC.DTR);
...
ParticleCricle.prototype.init = function(){
  var This = this;
  this.spin = 0;
  this.color = JC.random(COLORS);
  this.radius = JC.random(0,10)>>0;
...
ParticleCricle.prototype.reset = function(){
  var halfW = w>>1;
  var halfH = h>>1;
  this.doc.x = JC.random(-halfW, halfW);
  this.doc.y = JC.random(-halfH, halfH);
  this.doc.alpha = 0;
...
function ParticleTriangle(){
  ...
  this.FOS = JC.random(['fill','stroke']);
...
ParticleTriangle.prototype.init = function(){
  var This = this;
  this.spin = JC.random(-3,3);
  this.color = JC.random(COLORS);
  this.radius = JC.random(4,16)>>0;
  var points = [
      {x: this.radius*Math.cos(0           ), y: this.radius*Math.sin(0           )},
      {x: this.radius*Math.cos(120 * JC.DTR), y: this.radius*Math.sin(120 * JC.DTR)},
      {x: this.radius*Math.cos(240 * JC.DTR), y: this.radius*Math.sin(240 * JC.DTR)}
  ];
  this.doc.drawCall(function(ctx){
    ctx.beginPath();
    ctx.fillStyle = This.color;
...
ParticlePlus.prototype.init = function(){
  var This = this,
    scale = JC.random(0.5,1),
    ww = 6*scale,hh = 30*scale;
  this.spin = JC.random(-3,3)>0?JC.random(2,3):JC.random(-3,-2);
  this.color = JC.random(COLORS);
  this.radius = JC.random(4,16)>>0;
  this.doc.drawCall(function(ctx){
    ctx.beginPath();
